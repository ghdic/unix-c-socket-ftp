...
int main(int argc, char * argv[])
{
	...
	char cpath[MAXBACKLOG][BUFSIZE] = {};	//고정 경로 저장
	char init_path[BUFSIZE] = {}; // 초기경로 저장

	int fd_max = 0;
	fd_set read_fds, temp_fds; // 읽기 감지
	int client_num = 0; // 연결된 클라이언트 수
	int client_fds[MAXBACKLOG]; // 참가 클라이언트들의 소켓번호

	getcwd(init_path, BUFSIZE);
	...
	while(1) {
		temp_fds = read_fds;
		if(select(fd_max + 1, &temp_fds, 0, 0, 0) == -1) {
			perror("Select error\n");
			exit(-1);
		}

		if(FD_ISSET(servsock, &temp_fds)) {
			// 연결 요청 수락
			...
			if(client_num < MAXBACKLOG) {
				...
				fd_max = fd_max > clisock ? fd_max:clisock;
				memcpy(cpath[client_num], init_path, BUFSIZE);
				client_fds[client_num++] = clisock;
			...
			if(FD_ISSET(fd, &temp_fds)) {
				...
				if(res == 1) { // 클라이언트의 단절 확인
					...
					for(int j = i + 1; j < client_num; j++) {
						...
						memcpy(cpath[j], cpath[j + 1], BUFSIZE);
					}

					i--; // 하나가 줄었기 때문에 반복문이 제대로 돌기 위해 줄여줌
				}
			}
		}
	}
